...
\begin{algorithm}
  \begin{algorithmic}
   ...
   \STATE {\bfseries Input:} Model parameters $\Theta = \{\pi_{1}, P, \theta_1, \dots, \theta_k\}$, observed sequence $x_{1:T}$
   \STATE $\log p\left(z_{T} \vert x_{1:T}\right) \leftarrow 0_{K}$ \hfill Initialize messages.
   \FOR{t = T  - 1\dots 1}
   \FOR{k = 1 \dots K}
   $\log p\left(z_{t} = k \vert x_{1:T}\right) \leftarrow \text{lse}\left(\log p\left(z_{t + 1} = k \vert x_{1:T}\right)\right) + \log p_{\theta_{k}}\left(y_{t}\right) + \log \pi_{k}$
   \ENDFOR
   \ENDFOR
...
Consider the standard Bayesian HMM, which places
$\Dir\left(\alpha\right)$ priors on the rows of the transition probability
matrix, $P_{1}, \dots, P_{K}$ and conjugate priors on the remaining
elements of $\Theta$. The main idea of the sticky HMM is to introduce a
``stickiness'' parameter $\kappa$ to the Dirichlet prior\footnote{Here, $e_{k}
  := \left(0, 0, \dots, 0, 1, 0, \dots, 0\right)$}: $P_{k} \sim
\Dir\left(\alpha + \kappa e_{k}\right)$. This means that draws from the prior on
$P$ will have larger weight along the diagonal, depending on the size of
$\kappa$, and since diagonal elements correspond to self-transitions, chains
drawn from the prior will be ``sticky.''

To draw samples from $p\left(\left(z_{it}\right), \Theta \vert
...
place of the $M$-step, we draw from $p\left(\Theta \vert \left(z_{it}\right),
\left(x_{it}\right)\right)$. We describe these conditionals in more detail
below.
