...
$\text{HDP}\left(\alpha, H\right)$ prior. Note that a $\DP\left(\alpha,
...
$\text{HDP}\left(\alpha\right)$ prior allows sharing of states across $P_{k}$.
...
$\DP$ base measure $Q = \sum_{j = 1}^{\infy} \beta_j \delta_{\theta_j}$ where
the sequence $\left(\beta_j\right)$ is drawn from a
$\text{GEM}\left(\alpha\right)$ distribution. Cosider a modified base measure,
$Q_k = \sum_j = 1}^{\infy} \left(\beta_j + \indic{k =
...
\begin{align*}
  ...
  Q_k \vert \left(\beta_j\right), \left(\theta_j\right) &\sim \sum_{j = 1}^{\infy} \left(\beta_j + \indic{k = j}\kappa\right)\delta_{\theta_j}\right) \\
  \left(\beta_j\right) &\sim \text{GEM}\left(\alpha\right)
