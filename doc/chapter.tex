...
For $t > 1$, define the \textit{one-step-ahead} and \textit{update} means and
covariances by
\begin{align}
...
\begin{algorithmic}
  \STATE {\bfseries Input:} Model parameters $\Theta = \{A, C, Q, R\}$ and
    ...
    \STATE $\mu_{0} = 0, \Sigma_{0} = Q$ \hfill initialize distribution of
    $z_{0}$.
    \FOR{$t = 1 \dots T$}
    \STATE $\mu_{t \vert t - 1} = A\mu_{t - 1}$ \hfill predict step
    \STATE $\Sigma_{t \vert t - 1} = A \Sigma_{t - 1} A^{T} + Q$
    \STATE $\mu_{t} = \mu_{t \vert t - 1} + K_{t}\left(y_{t} - C\mu_{t \vert t - 1}\right)$ \hfill update step
    \STATE $\Sigma_{t} = \left(I - K_{t}C\right)\Sigma_{t \vert t - 1}$
    \STATE where $K_{t} &= \left(\Sigma_{t \vert t - 1}^{-1} + C^{T}R C\right)^{-1} C^{T}R^{-1}$
    \ENDFOR
