...
\begin{algorithm}
   \caption{Normalize log probabilities}
   \label{alg:normalize_log}
   \begin{algorithmic}
     \STATE {\bfseries Input:} Unnormalized log probabilities $\log \tilde{p} = \left(\log \tilde{p}_{1}, \dots, \log \tilde{p}_{K}\right)$
     \STATE $m \leftarrow \max\left(\log \tilde{p}\right)$ \hfill Avoid numerical underflow
     \STATE $\log{p} \leftarrow \log \tilde{p} - \left(m + \log\left(\sum\exp{\log \tilde{p}_{k} - m}\right)\right)$
     \STATE {\bfseries Output:} Normalized $\log p$, such that $\sum \exp{\log p_{k}} = 1$.
   \end{algorithmic}
\end{algorithm}
...
\begin{algorithmic}
  \STATE {\bfseries Input:} Model parameters $\Theta = \{\pi_{1}, P, \theta_1, \dots, \theta_K\}$,
  observed sequence $x_{1:T}$
  ...
  \STATE $\log p\left(z_{1} \vert x_{1}\right) \leftarrow \texttt{normalize\_log\_space}\left(\log \tilde{p}\left(z_{t} \vert x_{1}\right)\right)$ \hfill Use algorithm \ref{alg:normalize_log}
  \FOR{$t = 2 \dots T$}
  ...
  \STATE $\log p\left(z_{t} \vert x_{1:t}\right) \leftarrow \texttt{normalize\_log\_space}\left(\log \tilde{p}\left(z_{t} \vert x_{1:t}\right)\right)$
  \ENDFOR
...
\vert x_{i(1:T)}\right)$ after applying EM. Rows correspond to individual
species, while columns are samples sorted by time. The three main panels are the
three subjects in the study. Here, rows $i$ are ordered according to a
hierarchical clustering on the estimated sequences $\left(p\left(z_{i1} \vert
x_{i(1:T)}\right), \dots, p\left(z_{iT} \vert x_{i(1:T)}\right)\right)$.
