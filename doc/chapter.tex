...
alternative model -- distinguish the $k^th$ state from the rest

Here, $H$ is the base measure.

$P_k ~ Q_k$
$P_k &\sim Q$
$P_k &\sim HDP-HMM\left(\alpha, H\right)$
$P_k &\sim \DP\left(\alpha, Q\right)$
$Q &\sim DP\left(\alpha, H\right)$
$Q_k &\sim \sum_{j = 1}^{\infty} \left(\beta_j + \delta \indic{j = k}\right) \delta_{\theta_j}$
inference with block sampling
