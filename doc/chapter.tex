...
\begin{align*}
  ...
  \Sigma_{t \vert t - 1}^{x} &= \Covarag{x_t \vert x_{0:\left(t - 1\right)}} \\
...
we simply describe the updates necessary to compute $M_t$ and $u_t$. During the
forwards scans, the required calculation is

while for backwards scans, the analogous updates are


Code implementing these updates is available at
\url{https://github.com/krisrs1128/tsc_microbiome/tree/master/src/scan}. 
We have also provided an example application to a real microbial abundance time
series, whose results are provided in Figure \ref{fig:}.
