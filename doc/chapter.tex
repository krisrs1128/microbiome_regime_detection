...
\begin{align*}
  m_{t - 1, t}\left(z_{t}\right) &= \sum_{z_{t - 1} = 1}^{K} \psi_{t - 1, t}\left(z_{t - 1}, z_t\right) \psi_{t}\left(z_t, x_t\right) \\
...
As demonstrated in the HMM example, the belief propagation abstraction can be
used to compute complex marginals in a way that avoids what can otherwise become
tedious calculation. Consider again the Switching Linear Dynamical System
example. 
