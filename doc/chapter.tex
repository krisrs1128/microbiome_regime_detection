...
\begin{figure}[ht]
  ...
  \caption{Each panel represents displays the posteriors at one iteration of the
    sampler. Posteriors for the processes associated with different $z_i$, are
    displayed over the full data range, even when the associated timepoints
    $t^k$ occupy a relatively restricted range. \label{fig:igp_abt_fits}}
\end{figure}
...
respectively. However, there is typically no differentiation between the two
spikes, even though they have very different magnitudes.
...
\begin{figure}[ht]
  \centering
  \includegraphics[width=\textwidth]{figure/igp_abt_cooccurrence}
  \caption{To bypass the label switching problem, we can count how often two
    ...
    switching). We see clear antibiotic regime groups. \label{fig:igp_abt_states}}
\end{figure}
