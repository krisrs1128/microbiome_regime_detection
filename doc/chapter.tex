...
full parameter set consists of $\{\left(\theta_k\right)_{k = 1}^{\infy},
...
\begin{align*}
  \beta &\sim \Dir\left(\frac{\gamma}{L} + \overbar{m}_{\cdot 1}, \dots, \frac{\gamma]{L} + \overbar{m}_{\cdot L}\right),
\end{align*}
...
\begin{align*}
m_{jk} &\sim \sum_{n = 1}^{n_{jk}} \Bern\left(\frac{\alpha \beta_{k} + \kappa\indic{j = k}}{n + \alpha \beta_{k} + \kappa \indic{j = k}}
\end{align*}
...
\end{align*}
where $\rho = \frac{\kappa}{\alpha + \kappa}$. $\overbar{m}_{jk}$ is set
deterministically, according to
\begin{align*}
  \overbar{m}_{jk} &= \begin{cases}
    m_{jk} &\text{for } j \neq k\\
