...
\begin{algorithm}
  \begin{algorithmic}
   ...
   \STATE {\bfseries Input:} Model parameters $\Theta = \{\pi_{1}, P, \theta_1, \dots, \theta_k\}$, observed sequence $x_{1:T}$, forwards pass output $\left(\log p\left(z_{t} \vert x_{1:t}\right)\right)_{t = 1}^{T}$
   \STATE
  \end{algorithmic}
\end{algorithm}
...
``stickiness'' parameter $\kappa$ to the ordinary transition probability matrix.
